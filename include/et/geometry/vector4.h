...
template<typename T>
inline vector4<T> lerp(const vector4<T>& v1, const vector4<T>& v2, const T& t) {
	return v1 * (static_cast<T>(1) - t) + v2 * t;
}
...
template <typename T>
inline vector4<float> vector4ToFloat(const vector4<T>& v) {
	return vector4<float>(static_cast<float>(v.x), static_cast<float>(v.y), static_cast<float>(v.z), static_cast<float>(v.w));
}
