...
Texture2D<float2> normal : DECLARE_TEXTURE;
Texture2D<float4> shadow : DECLARE_TEXTURE;
Texture2D<float4> emissiveColor : DECLARE_TEXTURE;
...
float4 fragmentMain(VSOutput fsIn) : SV_Target0
{
	...
	baseColor.GetDimensions(sampledLevel, w, h, levels);

	float delta = 0.5;
	float4 sX = baseColor.SampleLevel(LinearClamp, fsIn.texCoord0, sampledLevel);
	float4 s0 = baseColor.SampleLevel(LinearClamp, fsIn.texCoord0 + float2(-delta / w, -delta / h), sampledLevel);
	float4 s1 = baseColor.SampleLevel(LinearClamp, fsIn.texCoord0 + float2( delta / w, -delta / h), sampledLevel);
	float4 s2 = baseColor.SampleLevel(LinearClamp, fsIn.texCoord0 + float2(-delta / w,  delta / h), sampledLevel);
	float4 s3 = baseColor.SampleLevel(LinearClamp, fsIn.texCoord0 + float2( delta / w,  delta / h), sampledLevel);
	...
	float averageLuminance = emissiveColor.SampleLevel(LinearClamp, fsIn.texCoord0, 10.0).x;

	baseColor.GetDimensions(0.0, w, h, levels);
	float2 texel = float2(1.0 / w, 1.0 / h);

	float3 hdrColor = baseColor.Sample(LinearClamp, fsIn.texCoord0).xyz;
...
#	if (ENABLE_COLOR_GRADING)
	{
		...
		float3 sample0 = shadow.SampleLevel(LinearClamp, float2(u + z0, v), 0.0).xyz;
		float3 sample1 = shadow.SampleLevel(LinearClamp, float2(u + z1, v), 0.0).xyz;
...
#	endif
	...
	float2 velocitySample = normal.Sample(LinearClamp, fsIn.texCoord0);
	float3 historySample = emissiveColor.Sample(LinearClamp, fsIn.texCoord0 + velocitySample).xyz;

	float3 s00 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(-1, -1)).xyz;
	float3 s01 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2( 0, -1)).xyz;
	float3 s02 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(+1, -1)).xyz;
	float3 s10 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(-1, 0)).xyz;
	float3 s11 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2( 0, 0)).xyz;
	float3 s12 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(+1, 0)).xyz;
	float3 s20 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(-1, +1)).xyz;
	float3 s21 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2( 0, +1)).xyz;
	float3 s22 = baseColor.Sample(LinearClamp, fsIn.texCoord0, uint2(+1, +1)).xyz;
