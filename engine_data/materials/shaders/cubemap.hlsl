...
float4 fragmentMain(VSOutput fsIn) : SV_Target0
{
	...
	for (uint i = 0; i < samples; ++i)
	{
		...
		float3 l = tX * h.x + tY * h.y + n * h.z;
		result += step(0.0, dot(l, n)) * baseColorTexture.SampleLevel(baseColorSampler, l, 4.0).xyz;
	}
	return float4(result / float(samples), 1.0);
	...
	for (uint i = 0; i < samples; ++i)
	{
		...
		float level = clamp(0.5 * log2(1.0 + sampleSolidAngle / cubemapSolidAngle), 0.0, 8.0);
